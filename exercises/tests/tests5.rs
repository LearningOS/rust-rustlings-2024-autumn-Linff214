...
///
/// The `address` must contain a mutable reference to a valid `u32` value.
/// This function assumes that the address is valid and points to a unique,
/// mutable `u32`. Calling this function with an invalid or non-unique address
/// may lead to undefined behavior.
unsafe fn modify_by_address(address: usize) {
    // SAFETY: We are assuming that the address provided is a valid mutable
    // reference to a `u32` and it's safe to modify it.
    unsafe {
        let ptr = address as *mut u32;
        *ptr = 0xAABBCCDD; // Modifying the value at the address
    }
}
